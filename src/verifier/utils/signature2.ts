...
export async function create_cyber_signature(address: Address, mint_eligibility: boolean, data: string): Promise<Hex> {
  ...
  const { r, s, v } = await sign({
    ...
    privateKey: process.env.VERIFIER_PRIVATE_KEY2 as `0x${string}`,
  });
